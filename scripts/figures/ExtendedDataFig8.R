...
ggplot(detection_df_grouped)+ 
  ...
  scale_color_manual(values=colors,labels=c(1:5)) + 
  guides(color=guide_legend(nrow=1)) + theme(legend.key.size=unit(.7,'lines')) + labs(color="Strategy") +theme(legend.margin=margin(0,0,0,0),legend.box.margin=margin(0,0,0,0)) + 
  facet_rep_grid(rows=vars(travel_rate),cols=vars(scenario),labeller = label_parsed,repeat.tick.labels = T)

ggsave("figures/ExtendedDataFig8.pdf",width=180,units="mm",height=100)
