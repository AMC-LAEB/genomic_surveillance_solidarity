...
inc_df = detection_df %>%
  group_by(strategy) %>%
  dplyr::summarize(Lo = quantile(threshold_infections,0.025,na.rm=T),
                   Hi=quantile(threshold_infections,0.975,na.rm=T),
                   Mean = mean(threshold_infections,na.rm=T),
                   Q25=quantile(threshold_infections,0.25,na.rm=T),
                   Q75=quantile(threshold_infections,0.75,na.rm=T),
                   LocalDetect_Continent=length(which(continents[detection_country]==onsetcontinent))/length(onsetcontinent))

time_df = detection_df %>%
  group_by(strategy) %>%
  dplyr::summarize(Lo = quantile(threshold_day,0.025,na.rm=T),
                   Hi=quantile(threshold_day,0.975,na.rm=T),
                   Mean = mean(threshold_day,na.rm=T),
                   Q25=quantile(threshold_day,0.25,na.rm=T),
                   Q75=quantile(threshold_day,0.75,na.rm=T),
                   LocalDetect_Continent=length(which(continents[detection_country]==onsetcontinent))/length(onsetcontinent))
...
time = ggplot(detection_df_grouped)+ 
  ...
  theme(legend.position = 'top')+ theme(legend.key.width = unit(.2,"cm")) + xlab(expression("Variant"~ italic("R"[e]))) + ylab("Days until variant reaches\nvariant proportion threshold")+ 
...
inc = ggplot(detection_df_grouped) + 
  ...
  geom_errorbar(aes(x=factor(variant_r0),ymin=Lo,ymax=Hi,group=strategy,col=strategy),position=position_dodge(width=.85),linewidth=width_lo) + thm +
  scale_color_manual(values=cols_strategies,labels=c("2022","A","B","C","D")) + 
  theme(legend.position = 'top') + xlab(expression("Variant"~ italic("R"[e]))) + ylab("Global variant infections by day\nvariant proportion threshold reached") + theme(legend.margin=margin(0,0,0,0),legend.box.margin=margin(-10,0,-10,0)) + 
  geom_errorbar(width=0,aes(x=factor(variant_r0),ymin=Q25,ymax=Q75,group=strategy,col=strategy),position=position_dodge(width=.85),linewidth=width_hi) + 
  geom_point(aes(x=factor(variant_r0),y=Mean,group=strategy,col=strategy),position=position_dodge(width=0.85),cex=1,pch=21,fill='white') +
...
time+inc +plot_layout(design='A\nB') & plot_annotation(tag_levels='a') & theme(plot.tag = element_text(size=8,face='bold'))

ggsave("SuppFig_10.pdf",width=2080,height=1700,units='px',dpi=320)


u = detection_df[detection_df$strategy=="min0maxInf",] %>%
